        ...
        <div class="col-md-12">
                <div class="mb-5 rounded-xl d-flex flex-column justify-content-end shelf_cover  bg-opacity-50" style="background-image: url(/images/shelf.jpg);">
                    <div class="page-cover d-flex flex-column justify-content-between h-100">
                        ...
                        <div id="subjects" class="w-75 mx-auto h-25">
                            <a class="btn btn-sm m-1 px-3 rounded-pill {{!isset($_GET['subject']) || $_GET['subject'] == '0'  ? 'active text-warning btn-dark' : 'btn-secondary'}}" href="/explore?subject=0&level={{@$_GET['level']}}&sort={{@$_GET['sort']}}&search={{@$_GET['search']}}">ทั้งหมด</a>
                            @foreach($subjects as $subject)
                                <a class="btn btn-sm m-1 px-3 rounded-pill {{isset($_GET['subject']) && $_GET['subject'] == $subject->id ? 'active text-warning btn-dark' : 'btn-secondary'}}" href="/explore?subject={{$subject->id}}&level={{@$_GET['level']}}&sort={{@$_GET['sort']}}&search={{@$_GET['search']}}">{{$subject->title}}</a>
                            @endforeach
                        </div>
                        <div class="d-flex flex-row justify-content-end h-25 pb-5">
                ...
                <div class="mt-3 row mb-5">
                    @if(count($contents) > 0)
                        ...
                        @foreach($contents as $x => $content)
                            <li class="col-md-3 p-2" href="{{url('/view/'.$content->id)}}">
